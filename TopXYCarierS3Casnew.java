...
public class TopXYCarierS3Casnew extends Configured implements Tool {

	static final String KEYSPACE = "aviation";
    static final String OUTPUT_COLUMN_FAMILY = "q2_3";	
	
    public static void main(String[] args) throws Exception {
        int res = ToolRunner.run(new Configuration(), new TopXYCarierS3Casnew(), args);
        System.exit(res);
    }
    ...
    public int run(String[] args) throws Exception {
        ...
        jobA.setJarByClass(TopXYCarierS3Casnew.class);
        ...
        String query = "UPDATE " + KEYSPACE + "." + OUTPUT_COLUMN_FAMILY + " SET airport = ?, carrier = ?, destination = ?, mean = ? ";
        CqlConfigHelper.setOutputCql(jobB.getConfiguration(), query);
        jobB.setOutputFormatClass(CqlOutputFormat.class);
       

        jobB.setJarByClass(TopXYCarierS3Casnew.class);
    ...
    public static class TopCariersReduce extends Reducer<Text, Text,  Map<String, ByteBuffer>, List<ByteBuffer>> {
        ...
        public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
        	...
        	for ( TextArrayWritable t : topDict.values() ) {
				...
				keys.put("comp", ByteBufferUtil.bytes(airport.toString()+"/"+destination.toString()+"/"+tuple[1].toString()));
