...
public class TopXYCarierS3C1 extends Configured implements Tool {

	static final String KEYSPACE = "aviation";
    static final String OUTPUT_COLUMN_FAMILY = "q2_3_new";	
	
    public static void main(String[] args) throws Exception {
        int res = ToolRunner.run(new Configuration(), new TopXYCarierS3C1(), args);
        System.exit(res);
    }
    ...
    public int run(String[] args) throws Exception {
        ...
        jobA.setJarByClass(TopXYCarierS3C1.class);
    ...
    public static class AirportStatMap extends Mapper<Object, Text, Text, FloatWritable> {
	...
	@Override
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
			try {
				...
				mkey = new Text(tokens[4].trim() + ";" + tokens[6].trim() + ";"+ tokens[2].trim());
				Float arr_delay = Float.parseFloat(tokens[13].trim());
				context.write(mkey, new FloatWritable(arr_delay));
			} catch (Exception e) {
				e.printStackTrace();
			}
    ...
    public static class AirportStatReduce extends Reducer<Text, FloatWritable, Text, Text> {
        ...
        public void reduce(Text key, Iterable<FloatWritable> values, Context context) throws IOException, InterruptedException {
			...
			destination = new Text(tokens[1].trim());
			carier = new Text(tokens[2].trim());
			mkey = new Text(airport.toString() + "/" + destination.toString());
			context.write(mkey, new Text(carier + ";" + Float.toString(sum/count)));
    ...
    public static class TopCariersReduce extends Reducer<Text, Text,  Map<String, ByteBuffer>, List<ByteBuffer>> {
        ...
        public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
        	//HashMap<Text, TreeMap<Float, TextArrayWritable>> bigStat = new HashMap<Text, TreeMap<Float, TextArrayWritable>>();
        	TreeMap<Float, TextArrayWritable> topDict = new TreeMap<Float, TextArrayWritable>();
        	
        	for ( Text value : values ) {
        		String[] tokens = value.toString().split(";");
        		Float mean = Float.parseFloat(tokens[1]);
        		String carier_name = tokens[0];
        		Text mkey = new Text(key);
        		String[] v = new String[]{mkey.toString(), carier_name, mean.toString()};
        		//topDict = new TreeMap<Float, TextArrayWritable>();
        		topDict.put(mean, new TextArrayWritable(v));
        		if (topDict.size() > N) {
        	...
        	for ( TextArrayWritable t : topDict.values() ) {
        		...
        		String[] tokens = tuple[0].toString().split("/");	
        		Text airport = new Text(tokens[0]);
				Text destination = new Text(tokens[1]);
				Map<String, ByteBuffer> keys = new LinkedHashMap<String, ByteBuffer>();
				keys.put("airport", ByteBufferUtil.bytes(airport.toString())); //+"/"+destination.toString()+"/"+tuple[1].toString()
				keys.put("destination", ByteBufferUtil.bytes(destination.toString()));
				keys.put("carrier", ByteBufferUtil.bytes(tuple[1].toString()));
				List<ByteBuffer> variables = new ArrayList<ByteBuffer>();
				//variables.add(ByteBufferUtil.bytes(airport.toString()));
				//variables.add(ByteBufferUtil.bytes(tuple[1].toString()));
				//variables.add(ByteBufferUtil.bytes(destination.toString()));
				variables.add(ByteBufferUtil.bytes(tuple[2].toString()));	
				context.write( keys, variables );
        	}
        	
        	/*
        	for ( Map.Entry<Text, TreeMap<Float,TextArrayWritable>> entry : bigStat.entrySet() ) {
				airport = entry.getKey();
				topDict = new TreeMap<Float, TextArrayWritable>();
				topDict = entry.getValue();
				NavigableSet<Float> nset = topDict.navigableKeySet();
				...
				//for ( TextArrayWritable el : nset.values() ) {
					...
					String[] tokens = tuple[0].toString().split("/");
					//airport = tuple[0];
					airport = new Text(tokens[0]);
					destination = new Text(tokens[1]);
					Map<String, ByteBuffer> keys = new LinkedHashMap<String, ByteBuffer>();
